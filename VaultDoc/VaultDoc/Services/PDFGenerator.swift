...
struct PDFGenerator {
    ...
    private static func drawHeader(in rect: CGRect, ctx: CGContext, item: Item) {
        ...
        "VaultDoc".draw(at: CGPoint(x: 30, y: 26), withAttributes: logoAttrs)
    ...
    private static func drawValueCard(in rect: CGRect, ctx: CGContext, item: Item, y: CGFloat) -> CGFloat {
        ...
        "DECLARED VALUE".draw(at: CGPoint(x: 50, y: y + 20), withAttributes: labelAttrs)
        ...
        if let ai = item.aiEstimate {
            "AI ESTIMATE".draw(at: CGPoint(x: 320, y: y + 20), withAttributes: labelAttrs)
    ...
    private static func drawDetailsGrid(in rect: CGRect, ctx: CGContext, item: Item, y: CGFloat) -> CGFloat {
        let fields: [(String, String)] = [
            ...
            ("Year Purchased", String(item.yearPurchased)),
            ("Purchase Price", CurrencyFormatter.format(item.purchasePrice)),
            ("Serial Number", item.serialNumber.isEmpty ? "—" : item.serialNumber),
            ("Notes", item.notes.isEmpty ? "—" : item.notes)
        ]
    ...
    private static func drawPhotos(in rect: CGRect, ctx: CGContext, item: Item, y: CGFloat, pdfCtx: UIGraphicsPDFRendererContext) -> CGFloat {
        ...
        "PHOTOS".draw(at: CGPoint(x: 30, y: y), withAttributes: headerAttrs)
    ...
    private static func drawDocumentList(in rect: CGRect, ctx: CGContext, item: Item, y: CGFloat) {
        ...
        "DOCUMENTS".draw(at: CGPoint(x: 30, y: y), withAttributes: headerAttrs)
        var currentY = y + 18
    ...
    private static func drawFooter(in rect: CGRect, ctx: CGContext, page: Int) {
        ...
        "Generated by VaultDoc  •  Confidential Insurance Document".draw(
